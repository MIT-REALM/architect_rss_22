...
def plot_comparison_mission_2():
    ...
    print(f"{(b32_df[mask].value > 0).sum()} out of 50 r32 trials satisfy STL")
    ...
    b64_df["method"] = "NLopt\n(n=64)"
    mask = b64_df.measurement == "STL Robustness"
    print(f"{(b64_df[mask].value > 0).sum()} out of 50 r64 trials satisfy STL")
    mask = b64_df.measurement == "Optimization time (s)"
    print(f"{b64_df[mask].value.mean()} r64 runtime")
    ...
    mask = cg_df.measurement == "STL Robustness"
    print(f"{(cg_df[mask].value > 0).sum()} out of 50 CG trials satisfy STL")
    print(f"{cg_df[mask].value.mean()} mean CG STL robustness")
    mask = cg_df.measurement == "Optimization time (s)"
    print(f"{cg_df[mask].value.mean()} CG runtime")
    ...
    rob_ax1.set_ylabel("")
    rob_ax1.set_xlabel("")
    rob_ax1.set_ylim([-7, 1.0])

    rob_ax2.set_title("")
    rob_ax2.set_ylabel("")
    rob_ax2.set_xlabel("")
    rob_ax2.set_ylim([-76, -73])
    ...
    time_ax1.set_ylabel("")
    time_ax1.set_xlabel("")
    time_ax1.set_ylim([900, 1100])
    time_ax1.set_yticks([900, 1000, 1100])

    time_ax2.set_title("")
    time_ax2.set_ylabel("")
    time_ax2.set_xlabel("")
    time_ax2.set_ylim([-1, 500])
    ...
    # sns.boxplot(x="method", y="value", data=results_df[mask], linewidth=1)
